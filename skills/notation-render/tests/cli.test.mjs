...
test("render(valid, svg) -> ok, output starts with <svg", () => {
  const r = render(validDsl, "svg");
  assert.equal(r.ok, true);
  assert.ok(r.output.startsWith("<svg "));
});

test("render(invalid, svg) -> not ok, empty output, diagnostics present", () => {
  const r = render(invalidDsl, "svg");
...
test("CLI exit codes: valid=0, invalid=1, bad-format=2", () => {
  const ok = runCli([join(examplesDir, "example-a.dsl"), "--format", "svg"]);
  assert.equal(ok.status, 0);
  assert.ok(ok.stdout.startsWith("<svg "));

  const bad = runCli([join(examplesDir, "invalid.dsl"), "--format", "svg"]);
